...
#remove participants that did not complete all session 2 vs 3
Participant <- subset(Participant, Participant != "P009" & Participant != "P040")
...
results <- foreach(i=1:n_subj, .combine = "rbind") %do% {
  ...
  datap1 <- subset(Data.trimmed, Participant == as.character(j) & Probability == 2 & Session == 2)
  datap2 <- subset(Data.trimmed, Participant == as.character(j) & Probability == 2 & Session == 3)
  datai1 <- subset(Data.trimmed, Participant == as.character(j) & Probability == 1 & Session == 2)
  datai2 <- subset(Data.trimmed, Participant == as.character(j) & Probability == 1 & Session == 3)
