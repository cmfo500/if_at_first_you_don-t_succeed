...
listOfPackages <- c("BlandAltmanLeh", "tidyverse", "dplyr", "magrittr", "lme4", "irr", "ggExtra", "psych", "ggplot2", "data.table", "stringr", "lmerTest", "dfoptim", "optimx", "stats", "GLMMadaptive", "EnvStats", "sjPlot", "emmeans", "patchwork", "sjPlot", "sjmisc", "tidyr", "broom", "LaplacesDemon", "broom.mixed", "DHARMa", "retimes", "BayesFactor", "data.table", "cowplot")
...
# Save plot
RT.plot
ggsave("plots/Procedural_learning.png", dpi = 1400, height = 9, width = 12, bg = "white")
...
tau.ISI.cutoff1 <- median(na.omit(log(Cor_data_ISI$Ex_gaussian_PVT_tau_1)))
tau.ISI.cutoff2 <- median(na.omit(log(Cor_data_ISI$Ex_gaussian_PVT_tau_2)))

# tau session 1
low_tau.ISI <- subset(Cor_data_ISI, log(Ex_gaussian_PVT_tau_1) <= tau.ISI.cutoff1)
high_tau.ISI <- subset(Cor_data_ISI, log(Ex_gaussian_PVT_tau_1) > tau.ISI.cutoff1)

cor.test(low_tau.ISI$Diff1_slopes, low_tau.ISI$Diff2_slopes)
cor.test(high_tau.ISI$Diff1_slopes, high_tau.ISI$Diff2_slopes)

# tau session 2
low_tau2.ISI <- subset(Cor_data_ISI, log(Ex_gaussian_PVT_tau_2) <= tau.ISI.cutoff2)
high_tau2.ISI <- subset(Cor_data_ISI, log(Ex_gaussian_PVT_tau_2) > tau.ISI.cutoff2)
...
tau.noISI.cutoff1 <- median(na.omit(log(Cor_data_noISI$Ex_gaussian_PVT_tau_1)))
tau.noISI.cutoff2 <- median(na.omit(log(Cor_data_noISI$Ex_gaussian_PVT_tau_2)))

# tau session 1
low_tau.noISI <- subset(Cor_data_noISI, log(Ex_gaussian_PVT_tau_1) <= tau.noISI.cutoff1)
high_tau.noISI <- subset(Cor_data_noISI, log(Ex_gaussian_PVT_tau_1) > tau.noISI.cutoff1)
...
# tau session 2
low_tau2.noISI <- subset(Cor_data_noISI, log(Ex_gaussian_PVT_tau_2) <= tau.noISI.cutoff2)
high_tau2.noISI <- subset(Cor_data_noISI, log(Ex_gaussian_PVT_tau_2) > tau.noISI.cutoff2)
...
grid <- plot_grid(BA_ISI, BA_noISI, ncol = 2, labels = c("A", "B"))

grid

ggsave("plots/Bland_Altman.plots6.png", dpi = 1200, height = 9, width = 12, bg = "white")
```
...
ISI.age.model <- lm(Diff2_slopes ~ Diff1_slopes*Age, data = Diff_ISI_trimmed) # no effect of age on reliability
summary(ISI.age.model)

noISI.age.model <- lm(Diff2_slopes ~ Diff1_slopes*Age, data = Diff_noISI_trimmed) # no effect of age on reliability
summary(noISI.age.model)
```
...
Split_half <- data.frame(Measure = c("Difference", "Ratio1", "Ratio2", "Regression slopes"),
                         ...
                         Split.half_ISI_1_R = round(c(SH_D1$estimate, SH_R1.1$estimate, SH_R1.2$estimate, SH_RS1$estimate),2),
                         N_ISI_2 = c(SH_D2$parameter+2, SH_R2.1$parameter+2, SH_R2.2$parameter+2, SH_RS2$parameter+2),
                         Split.half_ISI_2 = round(c(SH_D2$estimate, SH_R2.1$estimate, SH_R2.2$estimate, SH_RS2$estimate),2),
                         N_noISI_1 = c(SH_D3$parameter+2, SH_R3.1$parameter+2, SH_R3.2$parameter+2, SH_RS3$parameter+2),
                         Split.half_noISI_1 = round(c(SH_D3$estimate, SH_R3.1$estimate, SH_R3.2$estimate, SH_RS3$estimate),2),
                         N_noISI_2 = c(SH_D4$parameter+2, SH_R4.1$parameter+2, SH_R4.2$parameter+2, SH_RS4$parameter+2),
                         Split.half_noISI_2 = round(c(SH_D4$estimate, SH_R4.1$estimate, SH_R4.2$estimate, SH_RS4$estimate),2)) 
...
lapses <- DescTools::SpearmanRho(PVT_trimmed$lapses_1, PVT_trimmed$lapses_2, use = "pairwise.complete.obs", conf.level = .95)
mean_RT <- cor.test(PVT_trimmed$mean_PVT_1, PVT_trimmed$mean_PVT_2, method = "pearson", exact = FALSE)
reciprocal <- cor.test(PVT_trimmed$mean_recip_1, PVT_trimmed$mean_recip_2, method = "pearson", exact = FALSE)
median <- cor.test(PVT_trimmed$median_1, PVT_trimmed$median_2, method = "pearson", exact = FALSE)
tau <- cor.test(PVT_tau_trimmed$Ex_gaussian_PVT_tau_1, PVT_tau_trimmed$Ex_gaussian_PVT_tau_2, method = "pearson", exact = FALSE)

PVT_reliability <- data.frame(Measure = c("Lapses", "Mean RT", "Median", "Reciprocal",  "Ex-gaussian Tau"),
                              Test_retest = round(c(lapses[1], mean_RT$estimate, median$estimate, reciprocal$estimate,
                                                    tau$estimate),3))
...
### Summary Split-half reliability PVT
...
lapses1 <- DescTools::SpearmanRho(PVT_trimmed_SH$lapses_1_Even, PVT_trimmed_SH$lapses_1_Odd, use = "pairwise.complete.obs", conf.level = .95)

lapses2 <- DescTools::SpearmanRho(PVT_trimmed_SH$lapses_2_Even, PVT_trimmed_SH$lapses_2_Odd, use = "pairwise.complete.obs", conf.level = .95)
...
PVT_Split_half <- data.frame(Measure = c("Lapses", "Mean", "Median", "Reciprocal"),
                             Split.half_1 = round(c(lapses1[1], mean_RT1$estimate, median1$estimate, reciprocal1$estimate),3),
                             Split.half_2 = round(c(lapses2[1], mean_RT2$estimate, median2$estimate, reciprocal2$estimate),3))
